<template>
    <ul class="cart-list sheet">
        <li v-for="pizza in pizzas" :key="pizza.id" class="cart-list__item">
            ...
            <AppCounter class="cart-list__counter" :count="pizza.quantity" :orange="true" :max="10"
                @decrement="decrement(pizza.id, pizza.quantity - 1)" @increment="increment(pizza, pizza.quantity + 1)" />
            ...
            <div class="cart-list__button">
                <router-link :to="`/${pizza.id}`" class="cart-list__edit">Изменить</router-link>
            </div>
        </li>
    </ul>
</template>
