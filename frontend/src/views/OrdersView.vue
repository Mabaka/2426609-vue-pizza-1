<template>
  <main class="layout">
    <div class="layout__content">
      ...
      <section v-for="order in profile_store.orders" :key="order.id" class="sheet order">
        <div class="order__wrapper">
          ...
          <div class="order__button">
            <button type="button" class="button button--border" @click="profile_store.order_drop(order.id)">
              Удалить
            </button>
          </div>
          <div class="order__button">
            <button type="button" class="button"
              @click="profile_store.order_add({
        ...
        <ul class="order__additional">
          <li v-for="misc in order.orderMisc" :key="misc.id">
            <img :src="getMiscImgById(misc.miscId)" width="20" height="30" :alt="misc.name" />
            <p>
...
import { getImageUrl, getFilteredPizzas, getFilteredMiscs } from "@/common/helpers";
...
const getPizzaPrice = (sauceId, sizeId, doughId, ingredients) => {  
  const saucePrice = data_store.sauce.find((sauce) => sauce.id === sauceId).price;
  const sizeMultiplier = data_store.sizes.find((size) => size.id === sizeId).multiplier;
  const doughPrice = data_store.dough.find((dough) => dough.id === doughId).price;

  let ingredientsPrice = 0;

  ingredients?.map((ing) => {data_store.ingredients.map((ingg) => {
...
const getIngredientString = (ingredients) => {
  ...
  ingredients?.map((item) => {data_store.ingredients.map((dataItem) => {
